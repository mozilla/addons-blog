...
describe(__filename, () => {
  describe('Asset Pipeline functions', () => {
    ...
    describe('updateKeys', () => {
      const key = 'assets/css/file1.css';
      ...
      it('Should update a given key with hash data', async () => {
        ...
        expect(ap.assetMap[key].hashedPath).toBe(
          'assets/css/file1.testhash.css'
        );
      });
    ...
    describe('getReplacementURL', () => {
      ...
      it('should provide a hashed path based on the input (with query string)', () => {
        const replacement = ap.getReplacementURL(
          '/foo/test.css?foo=1',
          fileObj
        );
    ...
    describe('cachebustAssets', () => {
      ...
      it('should rewrite a binary file to dest', async () => {
        const exists = await fs
          .stat(
            path.join(dest, ap.assetMap['assets/octoamo-sm.png'].hashedPath)
          )
          .catch(() => false);
      ...
      it('should rewrite a css file to dest', async () => {
        const exists = await fs
          .stat(path.join(dest, ap.assetMap['assets/css/file1.css'].hashedPath))
          .catch(() => false);
        expect(exists).not.toBe(false);
      });
      ...
      it('should have rewritten asset refs in css', async () => {
        const css = await fs.readFile(
          path.join(dest, ap.assetMap['assets/css/file1.css'].hashedPath),
          'utf8'
        );
        const { hashedPath } = ap.assetMap['assets/octoamo-sm.png'];

        expect(css).toContain(hashedPath);
        // We expect the same image path twice.
        expect(css.match(new RegExp(hashedPath, 'g')).length).toEqual(2);
      });
